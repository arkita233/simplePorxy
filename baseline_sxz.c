...
//========= Cache=====================
struct Pack{
    char* key; // context after GET, in front of HTTP version
    char* data; // cache
...
void storeData(char *data,struct Pack* p, int* num_item,int cache_size,int t, char* key, int* t_count);
//=========Function==================
...
int time_count = 0; // time of GET from cache 
//=========================================
...
void handle(int newsock, fd_set *set)
{
    ...
    if(k ==-1 || (k!=-1 && difftime(pack[k].time_expire,time(NULL))<=0))
    {
      ...
      if(ssl!=1) {
        ...
        storeData(backup,pack,&num_elements,CACHE_SIZE,t, key, &time_count);
        ...
        int t1=0;
        for(t1=0;t1<num_elements;++t1)
        {
            printf("===%s %d %d====\n",pack[t1].key,pack[t1].time_last,pack[t1].time_expire);
    ...
    else   // 3. if exist, send cache to client 
    {
                    ...
                    if(difftime(pack[k].time_expire,time(NULL))>0) {
                        ...
                        time_count++; // update the time_count, assign to the value which is called by GET
                          ...
                          n = write(newsock, backup, strlen(backup));
